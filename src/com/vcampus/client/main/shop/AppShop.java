...
public class AppShop extends JFrame {
    ...
    private static final String EMPTY_GOODS_DESCRIPTION = "这里应该是图书详细信息";
    private static final String CATEGORY_UNFILTERED = "所有图书";
    ...
    public void handleCategorySelection(String category) {
        ...
        if (list == null) {
            SwingUtils.showMessage(null, "抱歉，没有搜到符合条件的图书，管理员正在努力备货中...", "提示");
        } else {
    ...
    private void purchase() {
        ...
        for (int i=0;i<tblCart.getRowCount();i++)
        {
            int result=submitPurchase(cardNumber, App.session.getUserType(),
                    tblCart.getValueAt(i, 0) + "@" + tblCart.getValueAt(i,2)+"@" + tblCart.getValueAt(i,3),totalCost);
            if (result==0)
            {
                int num= (int) tblCart.getValueAt(i,2);BigDecimal per=(BigDecimal) tblCart.getValueAt(i,3);
                System.out.println(totalCost);
                GoodsHistory thistime=new GoodsHistory(tblCart.getValueAt(i, 0).toString(),num,totalCost);
                insertPurchaseHistory(thistime);
            }
            //per.multiply(BigDecimal.valueOf(num))
            checker+=result;
        }
        if (checker==0)
        {
    ...
    private void addToCart(int x)
    {
        ...
        if (row == -1) {
            JOptionPane.showMessageDialog(null, "未选中图书", "错误", JOptionPane.ERROR_MESSAGE);
            return;
        }
    ...
    public AppShop() {
        ...
        cmbGoodsCategory.addItem("教材");
        cmbGoodsCategory.addItem("文学");
        cmbGoodsCategory.addItem("历史");
